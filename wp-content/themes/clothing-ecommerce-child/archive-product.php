...
$clothiq_shop_page_id = function_exists( 'wc_get_page_id' ) ? wc_get_page_id( 'shop' ) : 0;
if (
    $clothiq_shop_page_id > 0
    && is_shop()
    && ! is_search()
    && ! is_product_category()
    && ! is_product_tag()
    && clothiq_is_elementor_built( $clothiq_shop_page_id )
) {
    echo '<div class="clothiq-elementor-shop-content">';
    echo clothiq_get_elementor_content( $clothiq_shop_page_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    echo '</div>';
    get_footer();
    return;
}
